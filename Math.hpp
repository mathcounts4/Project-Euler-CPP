...
template<class T> Make_u<T> my_abs(T const t);

// for primitive types - checks if it's a power of 2
template<class T> bool is_pow_2(T const & t);

// my_gcd: always returns an unsigned value
template<class X, class Y> Common_u<X,Y> my_gcd(X const x, Y const y);

// my_lcm: always returns an unsigned value
template<class X, class Y> Common_u<X,Y> my_lcm(X const x, Y const y);

// my_pow: returns an object of type Base
template<class Base, class Exp> Base my_pow(Base b, Exp e);

// mod_inv: computes z such that x*z = m*k+1 for some integer m satisfying 0 <= z < |y|.
// If there is no such z this halts execution
template<class X, class Y> Common_u<X,Y> mod_inv(X const & x, Y const & y);
...
template<class X, class Y> Common_u<X,Y> mod_inv(X const & x, Y const & y) {
    ...
    return prev[0] ? negative[0]^(x<X(0)) ? mod-prev[0] : prev[0] : 0;
}
