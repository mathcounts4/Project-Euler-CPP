...
First, we show that, $\forall C\in\N$, given an infinite 2-D lattice of equally-spaced points, where each point is colored one of $C$ colors, it is always possible to find 3 points of the same color, where the 3 points form the top-left, bottom-left, and bottom-right corners of a square:
...
In particular, $\exists f : \N \mapsto \N$ with $f(C) = G$ meaning that, within any $G \times G$ grid of points colored by $C$ colors, the above pattern of 3 same-colored points can always be found. We show how to prove this for each number of colors $C$.

$C=1$ holds trivially, as all points are the same color, so $f(1)=2$.

$C=2$ (note: much of this proof is written generically for any $C$):

Find how many adjacent points are needed in a line to guarantee 2 of the same color: $C+1$:
...
Now, for all possible pairs within this line, append the point that would produce the desired patten of 3 points:
...
For simplicity, complete this into a square grid of side length $X=C+1$:
...
If the third point has the same color as the bottom two, we have found the pattern. Thus we can assume that it has the other color (assumes $C=2$ colors):
...
With $C$ colors, there is some $G_{C}$ where a $G_{C} \times G_{C}$ grid will contain the 3-corner pattern.\\
...
Thus, given an infinite grid colored by 2 colors, we can always find 4 corners of an axis-aligned square of the same color within a $Y \times Y$ grid, where $Y = X * G_{2}$.
...
Any finite pattern with integer coordinates can be found (at some scale) on an infinite $d$-dimensional grid of points colored by a finite number of colors.

First, fix a positive finite number of dimensions, $d$.

Define a ``pattern'' as a finite sequence of $n$ distinct points:\\
$\left\{\mathbf{p_i} = \{x_{(i,1)}, ..., x_{(i,d)}\}\right\}_{i=1...n}$ with integer coordinates in ($d$)-dimensional space.

We shall prove:

For any pattern in a $d$-dimensional space, where every point $\mathbf{P}$ in the integer lattice is colored by one of a finite number of colors $C$ via some function $c(\mathbf{P})$, there exists a ``box size'' $B$ such that, in any $B \times B \times ... \times B$ box of points selected from the integer lattice (starting at any $\mathbf{q}$), the pattern can be found (with some positive scaling factor $a$, at point $\mathbf{b}$) with all points sharing the same color:\\
