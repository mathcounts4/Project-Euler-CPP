	...
	private static class MousePanel extends JPanel {
		...
		private class MyBoard extends JPanel {
			...
			public void paintComponent(Graphics g) {
				...
				for (int i = 25; i != 550; i += 75) {
					g.drawLine(i-1,25,i-1,475);
					g.drawLine(i,25,i,475);
					g.drawLine(i+1,25,i+1,475);
					g.drawLine(25,i-1,475,i-1);
					g.drawLine(25,i,475,i);
					g.drawLine(25,i+1,475,i+1);
				}
				g.drawLine(475,174,505,174);
				g.drawLine(475,175,505,175);
				g.drawLine(475,176,505,176);
				g.drawLine(475,249,505,249);
				g.drawLine(475,250,505,250);
				g.drawLine(475,251,505,251);
				for (int i = tiles.size()-1; i != -1; i--) {
					...
					if (useTileType != TileType.DoNothing) {
						g.fillRect(27+75*x.posX,27+75*x.posY,w,h);
					}
				}
				if (mouseX < 475 && mouseX >= 25 && mouseY < 475 && mouseY >= 25) {
					int xBox = (mouseX-25)/75;
					int yBox = (mouseY-25)/75;
					if (isValidMove(currentTileType,xBox,yBox)) {
						...
						g.drawRect(27+75*xBox,27+75*yBox,w,h);
						g.drawRect(28+75*xBox,28+75*yBox,w-2,h-2);
						g.drawRect(29+75*xBox,29+75*yBox,w-4,h-4);
					}
				}
				if (arrowStart != -1) {
					int startX = 25+37+75*((arrowStart+arrowShift)%6);
					int startY = 25+37+75*((arrowStart+arrowShift)/6);
					int endX = 25+37+75*((arrowEnd+arrowShift)%6);
					int endY = 25+37+75*((arrowEnd+arrowShift)/6);
					g.setColor(Color.RED);
					if (startX == endX) {
